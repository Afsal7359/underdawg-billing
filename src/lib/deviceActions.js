 ...
 * ================================================================== */
import { fx, waDigits } from "./theme.js";
...
export function whatsapp(phone, text) {
  // waDigits turns "07911 123456" / "+44 7911 123456" into "447911123456",
  // which is the format wa.me requires (country code, no + and no leading 0).
...
export function receiptHTML(o, c, s = {}) {
  ...
  return `<div class="rc">
    <div class="rc-h"><div class="rc-store">${esc(s.storeName || "underdawg")}</div>
      <div class="rc-sub">${esc(s.address || "")}</div>${s.gstin ? `<div class="rc-sub">VAT ${esc(s.gstin)}</div>` : ""}</div>
    <div class="rc-meta"><span>${esc(o.no)}</span><span>${new Date(o.date).toLocaleString("en-GB")}</span></div>
    ...
    <div class="rc-tot">
      ...
      ${o.tax > 0 ? `<div><span>VAT</span><span>${fx(o.tax)}</span></div>` : ""}
...
export function statementHTML(title, subtitle, data, total) {
  ...
  return `<div class="rep"><h1>${esc(title)}</h1><div class="rep-sub">${esc(subtitle || "")}</div>
    ...
    <div class="rep-ft">Generated by underdawg Bill</div></div>`;
}
...
export function overviewHTML(ov, s = {}, periodLabel = "") {
  ...
  return `<div class="rep"><h1>${esc(s.storeName || "underdawg")} — Report</h1>
    <div class="rep-sub">${esc(periodLabel)} · generated ${new Date().toLocaleDateString("en-GB")}</div>
